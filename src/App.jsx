...
const CONFIG = [
  ...
  { line: 'levels = 1', tail: '# direct AUR dependencies; -1 analyses the whole closure' },
  { line: '' },
...
const namespaces = [
  {
    ...
    count: '119 rules',
    blurb: 'The pattern rules that read the diff. They run against variable-resolved command text and raw diff lines, and span every severity from INFO to FATAL.',
    cats: nsCategories.r,
  },
  {
    letter: 'C',
    ...
    count: '7 rules',
    blurb: 'Context rules about the diff itself: checksum and source-integrity heuristics, plus a couple of metadata and fetch facts.',
    cats: nsCategories.c,
  },
  {
    letter: 'D',
    ...
    count: '4 rules',
    blurb: 'Novelty against the dependency graph and the seed: has this dependency been seen in the AUR before.',
    cats: nsCategories.d,
  },
  {
    letter: 'S',
    name: 'Sabotage rules',
    count: '8 rules',
    blurb: 'Payloads aimed at the operator\'s machine rather than at getting something out of it: resource exhaustion, deletion, permission sabotage, service disruption, resource theft. Command-position matching keeps build-sandbox housekeeping apart from system damage.',
    cats: nsCategories.s,
  },
  {
    letter: 'X',
    ...
    count: '7 rules',
    blurb: 'The evasion technique, not the payload it hides. A word the tokenizer could not reduce to a literal is itself the signal: one rule covers the evasion surface of every payload rule at once.',
    cats: nsCategories.x,
  },
  {
    letter: 'P',
    ...
    count: 'weight 0',
    blurb: 'What the recipe declares, not what was found. Every P finding is INFO and checkable by the reader against the file itself. Reported, never credited: a signal an attacker can assert for free must not move a score.',
    cats: [
